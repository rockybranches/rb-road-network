...
void compute_population_table(float tons_person, std::string filename)
{
  ...
  text << " Tons needed by market (2015) & " << std::fixed << std::setprecision(3) <<
    tons_person*totals[1] << "  (" <<  tons_person << " tons/person) \\\\ \n";
  text << " Tons needed by market (2030) & " << std::fixed << std::setprecision(3) <<
    tons_person*totals[4] << "  (" <<  tons_person << " tons/person) \\\\ \n";
  text << " Increase in tons needed (2015 to 2030) & " << std::fixed << std::setprecision(3) <<
...
int main(int argc, char*argv[])
{
  ...
  std::string dbfn =
    rb::RB_DATA_PATH + "logs/" + "debug-" + std::string(std::getenv("USER")) +
