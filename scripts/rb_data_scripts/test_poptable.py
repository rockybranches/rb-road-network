...
usage = colored("\tUsage: ", "red", attrs=["bold"]) + colored(
    "python3 test_poptable.py /absolute/path/to/input/poptable.tex\r\n", "red"
)
...
def create_ptex_doc(input_str, output_fpath=None, genpdf=True):
    ...
    for old_files in [output_fpath + ".tex", output_fpath + ".pdf"]:
        if os.path.exists(old_files):
            backup_fpath = f"{old_files}.{int(time.time())}.bak"
            os.rename(old_files, backup_fpath)
            print(
                colored(
                    f"( saved a backup of an existing test to: {backup_fpath} )", "grey"
                )
            )
