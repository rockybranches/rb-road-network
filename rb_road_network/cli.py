...
def build_command(
    ...
    """Build the justPop.exe command as an argv list (safe for shell=False)."""
    return [
        str(exe_path),
        "-f", str(output),
        "-t", str(tons_per_person),
        f"--lat={lat}",
        f"--lon={lon}",
        f"--radius={radius}",
        f"--stride={stride}",
...
def save_script(output_path: Path, cmd: list, rb_src: Path) -> Path:
...
def batch(csv_file, tons_per_person, radius, stride, nthreads, zoom, output_dir, rb_src, render, no_exec):
    ...
    for row_num, row in enumerate(rows, start=2):
        ...
        try:
            ...
            raise click.ClickException(
                f"Row {row_num}: invalid lat/lon values "
                f"({row.get('lat')!r}, {row.get('lon')!r}): {exc}"
            )
