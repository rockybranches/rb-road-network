...
def plot_in_section_lines(
    axes,
    geosurfaces_plot: InSectionPlotLines
):

    multilines2d = geosurfaces_plot.multilines2d
    color_style = geosurfaces_plot.colors
    multilines_ids = geosurfaces_plot.ids
    plot_labels = geosurfaces_plot.plot_labels

    if color_style is None:
        colors = projected_line_random_colors * (int(len(multilines2d) / len(projected_line_random_colors)) + 1)
    else:
        colors = color_style

    for multiline_2d, classification_id in zip(multilines2d, multilines_ids):
...
def plot_geoprofiles(
    ...
    for ndx in range(geoprofiles.geoprofiles_num):
        ...
        if len(geoprofile.geosurfaces) > 0:
            ...
            #label_projected_lines = plot_addit_params["label_projected_lines"]
            #projected_lines_colors = plot_addit_params["projected_lines_class_colors"]
            for geosurface in geoprofile.geosurfaces:
                plot_in_section_lines(
                    axes_elevation,
                    geosurface
                )
